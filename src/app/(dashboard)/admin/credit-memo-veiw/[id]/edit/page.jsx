...
// BatchModal component – allows manual entry/editing of batch details.
...
export default function CreditNoteEditPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      let copiedData = null;
      const soData = sessionStorage.getItem("creditNoteData");
  ...
  // Fetch existing Credit Note data if editing.
  useEffect(() => {
    if (id) {
      axios
        .get(`/api/credit-note/${id}`)
        .then((res) => {
          // Update this block to work with your API response format.
          // If your API returns the credit note directly, use:
          const record = res.data && res.data._id ? res.data : {};
          setFormData({
            ...record,
  ...
  return (
    <div ref={parentRef} className="m-11 p-5 shadow-xl">
      <h1 className="text-2xl font-bold mb-4">
        {id ? "Edit Credit Note" : "Create Credit Note"}
      </h1>
      {/* Customer Section */}
